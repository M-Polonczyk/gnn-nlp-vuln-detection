#!/usr/bin/env python3
"""
Example usage of the DiverseVul dataset loader.

This script demonstrates how to load and work with the DiverseVul dataset
...
def display_statistics(stats) -> None:
    ...
    print("\n" + "=" * 50)
    print("DIVERSEVUL DATASET STATISTICS")
    print("=" * 50)
...
def demonstrate_ml_export(loader) -> None:
    """Demonstrate ML export functionality."""
    print("\n" + "=" * 50)
    print("MACHINE LEARNING EXPORT")
    print("=" * 50)

    # Export data for ML training
    ml_data = loader.export_for_ml(include_metadata=True)

    print(f"Exported {ml_data['total_samples']} samples for ML training")
    print(f"Functions: {len(ml_data['functions'])} entries")
    print(f"Targets: {len(ml_data['targets'])} labels")
    print(f"CWEs: {len(ml_data['cwes'])} CWE lists")
    print(f"Projects: {len(ml_data['projects'])} project names")

    if "metadata" in ml_data:
        metadata_count = sum(1 for m in ml_data["metadata"] if m is not None)
        print(f"Metadata: {metadata_count} entries with metadata")
...
def save_sample_data(loader, vulnerable_samples) -> None:
    ...
    output_path = Path("data/processed/diversevul_vulnerable_samples.json")
...
def main() -> None:
    """Main function demonstrating DiverseVul dataset usage."""

    # Example paths - adjust these to match your actual data locations
    dataset_path = "data/diversevul/dataset.json"  # Main dataset file
    metadata_path = "data/diversevul/metadata.json"  # Optional metadata file

    # Check if files exist
    if not Path(dataset_path).exists():
        print(f"Dataset file not found at {dataset_path}")
        print("Please download the DiverseVul dataset")
        print("Expected format: JSON file with list of function samples")
        print("Run create_sample_data() to generate test data")
        return

    # Initialize the loader
    print("Initializing DiverseVul dataset loader...")
    ...
    # Demonstrate ML export
    demonstrate_ml_export(loader)

    # Save sample data
    save_sample_data(loader, vulnerable_samples)

    print("\nDiverseVul dataset loading complete!")
...
def create_sample_data() -> None:
    ...
    sample_dataset = [
        ...
        {
            "func": "int safe_function(char *input) {\n    char buffer[256];\n    if (strlen(input) < 256) {\n        strcpy(buffer, input);\n        return process_buffer(buffer);\n    }\n    return -1;\n}",
            "target": 0,
            "cwe": [],
            "project": "test_project",
            "commit_id": "def456safe",
            "hash": 987654321,
            "size": 8,
            "message": "Safe function with bounds checking",
        },
    ]
    ...
    sample_metadata = [
        ...
        },
        {
            ...
            "CWE": "CWE-119",
            "CVE": "CVE-2023-5678",
            "bug_info": "Buffer overflow in padding check",
            "commit_url": "https://github.com/gnutls/gnutls/commit/7ad6162573ba79a4392c63b453ad0220ca6c5ace",
            "repo_url": "https://github.com/gnutls/gnutls",
        },
    ]

    # Create directories
    data_dir = Path("data/diversevul")
    ...
    print(f"Created sample DiverseVul data files in {data_dir}/")
